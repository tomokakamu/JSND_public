...
class Ai1wmke_FTP_Extension implements Ai1wmke_FTP_Interface {
	...
	public function __construct( $hostname, $username, $password, $directory = '/', $port = 21, $active = false, $ssl = false ) {
		if ( ! extension_loaded( 'ftp' ) ) {
			throw new Ai1wmke_Error_Exception( __( 'FTP Extension requires PHP FTP extension. <a href="https://help.servmask.com/knowledgebase/ftp-missing-in-php-installation/" target="_blank">Technical details</a>', AI1WMKE_PLUGIN_NAME ) );
		}
	...
	public function raw_list_folder( $folder_path ) {
		...
		try {
			if ( ( $items = @ftp_rawlist( $this->get_connection(), sprintf( '-tlA %s', $folder_path ) ) ) === false ) {
				throw new Ai1wmke_Error_Exception( __( 'FTP list has failed', AI1WMKE_PLUGIN_NAME ) );
			}
		} catch ( Ai1wmke_Error_Exception $e ) {
			throw $e;
		}
	...
	public function upload_file( $local_file_path, $remote_file_path, $file_size ) {
		...
		try {
			if ( @ftp_put( $this->get_connection(), $remote_file_path, $local_file_path, FTP_BINARY ) === false ) {
				throw new Ai1wmke_Error_Exception( __( 'FTP upload has failed', AI1WMKE_PLUGIN_NAME ) );
			}
		} catch ( Ai1wmke_Error_Exception $e ) {
			throw $e;
		}
	...
	public function download_file( $local_file_path, $remote_file_path ) {
		...
		try {
			if ( @ftp_get( $this->get_connection(), $local_file_path, $remote_file_path, FTP_BINARY ) === false ) {
				throw new Ai1wmke_Error_Exception( __( 'FTP download has failed', AI1WMKE_PLUGIN_NAME ) );
			}
		} catch ( Ai1wmke_Error_Exception $e ) {
			throw $e;
		}
		...
		if ( $this->handler === null ) {
			if ( $this->ssl ) {
				if ( ( $this->handler = @ftp_ssl_connect( $this->hostname, $this->port, 120 ) ) === false ) {
					throw new Ai1wmke_Connect_Exception( __( 'Unable to connect to FTPS server. Please check your FTPS hostname, port, and active mode settings. <a href="https://help.servmask.com/knowledgebase/ftp-error-codes/#invalid-hostname" target="_blank">Technical details</a>', AI1WMKE_PLUGIN_NAME ) );
				}
			} else {
				if ( ( $this->handler = @ftp_connect( $this->hostname, $this->port, 120 ) ) === false ) {
					throw new Ai1wmke_Connect_Exception( __( 'Unable to connect to FTP server. Please check your FTP hostname, port, and active mode settings. <a href="https://help.servmask.com/knowledgebase/ftp-error-codes/#invalid-hostname" target="_blank">Technical details</a>', AI1WMKE_PLUGIN_NAME ) );
				}
			}
