...
class Ai1wmke_URL_Import_Download {
	...
	public static function execute( $params, $url = null ) {
		...
		if ( ! isset( $params['file_url'] ) ) {
			throw new Ai1wm_Import_Exception( __( 'File URL is not specified.', AI1WMKE_PLUGIN_NAME ) );
		}

		// Validate file size
		if ( ! isset( $params['file_size'] ) ) {
			throw new Ai1wm_Import_Exception( __( 'File Size is not specified.', AI1WMKE_PLUGIN_NAME ) );
		}

		// Validate file ranges
		if ( ! isset( $params['file_ranges'] ) ) {
			throw new Ai1wm_Import_Exception( __( 'File Ranges is not specified.', AI1WMKE_PLUGIN_NAME ) );
		}

		$file_chunk_size = get_option( 'ai1wmke_url_file_chunk_size', AI1WMKE_URL_FILE_CHUNK_SIZE );
		...
		if ( $params['file_ranges'] === 'bytes' ) {
			...
			// Set progress
			Ai1wm_Status::info( __( 'Downloading from URL...', AI1WMKE_PLUGIN_NAME ) );
