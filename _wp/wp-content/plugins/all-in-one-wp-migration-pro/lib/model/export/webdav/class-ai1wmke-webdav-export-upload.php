...
class Ai1wmke_WebDAV_Export_Upload {
	...
	public static function execute( $params, $webdav = null ) {
		...
		if ( ( fseek( $archive, $params['archive_offset'] ) !== -1 )
			...
			try {
				...
				$webdav->upload_file_chunk( $file_chunk_data, $params['archive'], sprintf( '%s/%s', ai1wm_archive_folder(), ai1wm_archive_name( $params ) ), $params['file_chunk_number'] );
			...
			update_option( 'ai1wmke_webdav_timestamp', time() );
