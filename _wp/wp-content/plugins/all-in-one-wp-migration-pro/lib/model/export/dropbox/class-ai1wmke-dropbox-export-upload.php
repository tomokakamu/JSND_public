...
class Ai1wmke_Dropbox_Export_Upload {
		...
		if ( ( $archive = fopen( ai1wm_archive_path( $params ), 'rb' ) ) ) {
			...
			if ( ( fseek( $archive, $params['archive_offset'] ) !== -1 )
					...
					if ( $params['archive_size'] <= $file_chunk_size ) {
						try {
							if ( ai1wmke_is_incremental( 'dropbox' ) ) {
								$dropbox->upload_file( $file_chunk_data, sprintf( '/%s/incremental-backups/%s', $params['folder_path'], ai1wm_archive_name( $params ) ) );
							} else {
						...
						try {
							if ( ai1wmke_is_incremental( 'dropbox' ) ) {
								$dropbox->upload_file_chunk_commit( $file_chunk_data, sprintf( '/%s/incremental-backups/%s', $params['folder_path'], ai1wm_archive_name( $params ) ), $params['session_id'], $params['archive_offset'] );
							} else {
				...
				if ( defined( 'WP_CLI' ) ) {
					...
					Ai1wm_Status::info( sprintf( __( '<i class="ai1wmke-dropbox-icon"></i> Uploading <strong>%s</strong> (%s)<br />%d%% complete', AI1WMKE_PLUGIN_NAME ), $name, $size, $progress ) );
				}
			} else {

				// Set last backup date
				update_option( 'ai1wmke_dropbox_timestamp', time() );
