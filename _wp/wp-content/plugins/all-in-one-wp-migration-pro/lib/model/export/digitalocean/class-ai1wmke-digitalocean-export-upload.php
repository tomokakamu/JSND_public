...
class Ai1wmke_DigitalOcean_Export_Upload {
	...
	public static function execute( $params, $digitalocean = null ) {
		...
		if ( ! isset( $params['folder_name'] ) ) {
			throw new Ai1wm_Import_Exception( __( 'DigitalOcean Spaces Folder Name is not specified.', AI1WMKE_PLUGIN_NAME ) );
		}

		// Validate upload ID
		if ( ! isset( $params['upload_id'] ) ) {
			throw new Ai1wm_Import_Exception( __( 'DigitalOcean Spaces Upload ID is not specified.', AI1WMKE_PLUGIN_NAME ) );
		}
		...
		if ( ( fseek( $archive, $params['archive_offset'] ) !== -1 )
			...
			try {
				...
				if ( ai1wmke_is_incremental( 'digitalocean' ) ) {
					$file_chunk_etag = $digitalocean->upload_file_chunk( $file_chunk_data, sprintf( '/%s/incremental-backups/%s', $params['folder_name'], ai1wm_archive_name( $params ) ), $params['upload_id'], $params['bucket_name'], $params['region_name'], $params['file_chunk_number'] );
				} else {
					$file_chunk_etag = $digitalocean->upload_file_chunk( $file_chunk_data, sprintf( '/%s/%s', $params['folder_name'], ai1wm_archive_name( $params ) ), $params['upload_id'], $params['bucket_name'], $params['region_name'], $params['file_chunk_number'] );
				}
			...
			if ( defined( 'WP_CLI' ) ) {
				...
				Ai1wm_Status::info( sprintf( __( '<i class="ai1wmke-digitalocean-icon"></i> Uploading <strong>%s</strong> (%s)<br />%d%% complete', AI1WMKE_PLUGIN_NAME ), $name, $size, $progress ) );
			}
		} else {
			...
			if ( ai1wmke_is_incremental( 'digitalocean' ) ) {
				$digitalocean->upload_complete( $file_chunks, sprintf( '/%s/incremental-backups/%s', $params['folder_name'], ai1wm_archive_name( $params ) ), $params['upload_id'], $params['bucket_name'], $params['region_name'] );
			} else {
				$digitalocean->upload_complete( $file_chunks, sprintf( '/%s/%s', $params['folder_name'], ai1wm_archive_name( $params ) ), $params['upload_id'], $params['bucket_name'], $params['region_name'] );
			}

			// Set last backup date
			update_option( 'ai1wmke_digitalocean_timestamp', time() );
