...
class Ai1wmke_GDrive_Export_Upload {
	...
	public static function execute( $params, $gdrive = null ) {
		...
		if ( ! isset( $params['archive_size'] ) ) {
			throw new Ai1wm_Import_Exception( __( 'Google Drive Archive Size is not specified.', AI1WMKE_PLUGIN_NAME ) );
		}

		// Validate upload URL
		if ( ! isset( $params['upload_url'] ) ) {
			throw new Ai1wm_Import_Exception( __( 'Google Drive Upload URL is not specified.', AI1WMKE_PLUGIN_NAME ) );
		}
		...
		if ( ( fseek( $archive, $params['archive_offset'] ) !== -1 )
			...
			try {
				...
				if ( $params['upload_retries'] <= apply_filters( 'ai1wmke_default_api_retries', AI1WMKE_GDRIVE_API_RETRIES ) ) {
					if ( isset( $params['upload_backoff'] ) ) {
			...
			update_option( 'ai1wmke_gdrive_timestamp', time() );
