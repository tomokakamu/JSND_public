...
class Ai1wmke_Direct_Push_Start_Import {
	...
	public static function execute( $params, $direct_client = null, $token_client = null ) {
		...
		if ( defined( 'WP_CLI' ) ) {
			WP_CLI::log(
				sprintf(
					__( 'Remote import is starting. Remote status: %s', AI1WMKE_PLUGIN_NAME ),
					$status
				)
			);
		} else {
			Ai1wm_Status::info(
				sprintf(
					__( 'Remote import is starting <br/>', AI1WMKE_PLUGIN_NAME ) .
