...
class Ai1wmke_Reset_Tools_Controller {
	...
	public static function reset_tools( $params = array() ) {
		global $ai1wm_params;
				...
				if ( intval( $params['priority'] ) === key( $filters ) ) {
					foreach ( $hooks as $hook ) {
						try {
							...
							if ( defined( 'WP_CLI' ) ) {
								WP_CLI::error( sprintf( __( 'Unable to Proceed: %s', AI1WMKE_PLUGIN_NAME ), $e->getMessage() ) );
							} else {
								Ai1wm_Status::error( __( 'Unable to Proceed', AI1WMKE_PLUGIN_NAME ), $e->getMessage() );
								Ai1wm_Notification::error( __( 'Unable to Proceed', AI1WMKE_PLUGIN_NAME ), $e->getMessage() );
							}
							exit;
						}
					}
	...
	public static function reset_labels( $params ) {
		// Set archive
		$archive = null;
		...
		if ( isset( $archive, $reset_label ) ) {
			switch ( $reset_label ) {
				case 'plugins':
					$reset_type = __( 'Plugin Purge', AI1WMKE_PLUGIN_NAME );
					break;

				case 'themes':
					$reset_type = __( 'Theme Reset', AI1WMKE_PLUGIN_NAME );
					break;

				case 'media':
					$reset_type = __( 'Media Clean-Up', AI1WMKE_PLUGIN_NAME );
					break;

				case 'database':
					$reset_type = __( 'Reset Database', AI1WMKE_PLUGIN_NAME );
					break;

				default:
					$reset_type = __( 'Full Site Reset', AI1WMKE_PLUGIN_NAME );
			}

			try {
				Ai1wm_Backups::set_label( $archive, sprintf( __( 'Before %s - %s', AI1WMKE_PLUGIN_NAME ), $reset_type, date_i18n( 'F d, Y, h:i A' ) ) );
